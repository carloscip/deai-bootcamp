...
async function callOpenRouter(messages: any[], modelId: string, temperature = 0.7, maxTokens = 500) {
...
export async function generateJoke(params: JokeFormValues): Promise<JokeResult> {
  try {
    ...
    const jokeResponse = await callOpenRouter(jokeMessages, params.modelId, params.temperature, 500)
...
async function evaluateJoke(
  ...
  try {
    ...
    const evaluationResponse = await callOpenRouter(evaluationMessages, evaluationModelId, 0.5, 300)
